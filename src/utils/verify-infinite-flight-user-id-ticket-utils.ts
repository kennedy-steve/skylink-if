...
export class VerifyInfiniteFlightUserIdTicketUtils {
    public static getFreshTicketsCutoffDateTime(): Date {
        ...
        const staleByMilliseconds: number =
            Config.modelConstants.verifyInfiniteFlightUserIdTicket.STALE_BY_MINUTES *
            millisecondsPerMinute;
        const freshTicketsDateTime: Date = new Date(
            currentDateTime.getTime() - staleByMilliseconds
        );

        return freshTicketsDateTime;
    }

    public static checkIfFlightPassesAllChecks(
        flight: FlightEntry,
        ticket: VerifyInfiniteFlightUserIdTicket
    ): boolean {
    ...
    private static checkHeading(
        ...
        if (!isHeadingCorrect) {
            Logger.info(
                `Verification of ticket ${ticket.id} failed. Heading is off by ${headingDelta} degrees`
            );
        }
